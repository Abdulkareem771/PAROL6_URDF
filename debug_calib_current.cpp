...
private:
  ...
  void drawCoverage(cv::Mat &img) { int w=img.cols, h=img.rows; for(int i=1; i<3; ++i) { cv::line(img, cv::Point(i*w/3,0), cv::Point(i*w/3,h), cv::Scalar(0,255,255), 2); cv::line(img, cv::Point(0,i*h/3), cv::Point(w,i*h/3), cv::Scalar(0,255,255), 2); } for(int r=0; r<3; ++r) for(int c=0; c<3; ++c) { if(coverage[r*3+c]>0) { cv::putText(img, "OK", cv::Point(c*w/3+20, r*h/3+50), cv::FONT_HERSHEY_SIMPLEX, 1.5, cv::Scalar(0,255,0), 3); } else { cv::putText(img, "-", cv::Point(c*w/3+20, r*h/3+50), cv::FONT_HERSHEY_SIMPLEX, 1.5, cv::Scalar(0,0,255), 3); } } std::stringstream ss; int covCount=0; for(int k=0; k<9; ++k) if(coverage[k]>0) covCount++; ss << "Coverage: " << (int)((covCount/9.0)*100) << "%"; cv::putText(img, ss.str(), cv::Point(20, h-30), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0,255,0), 2); }   void checkSmartCapture(const std::vector<cv::Point2f> &points, const cv::Mat &color, const cv::Mat &ir, const cv::Mat &irGrey, const cv::Mat &depth, const std::vector<cv::Point2f> &pColor, std::vector<cv::Point2f> &pIr) { if(points.empty()) { isSteady=false; return; } cv::Point2f centroid(0,0); for(auto &p : points) centroid += p; centroid *= (1.0/points.size()); double dist = cv::norm(centroid - lastCentroid); if(dist < 3.0) { if(!isSteady) { steadyStart = node->now(); isSteady=true; } else if((node->now() - steadyStart).seconds() > 2.0) { if((node->now() - lastTime).seconds() > 3.0) { store(color, ir, irGrey, depth, pColor, pIr); lastTime = node->now(); steadyStart = node->now(); lastCentroid = centroid; OUT_INFO(node, "AUTO-CAPTURE TRIGGERED!"); } } } else { isSteady = false; } lastCentroid = centroid; }
  rclcpp::Time lastTime; rclcpp::Time steadyStart; cv::Point2f lastCentroid; cv::Point2f capturedCentroid; bool isSteady;
...
public:
  Recorder(const rclcpp::Node::SharedPtr &node, const ExecutorSharedPtr &executor, const std::string &path, const std::string &topicColor, const std::string &topicIr, const std::string &topicDepth,
      ...
      : node(node), executor(executor), circleBoard(circleBoard), boardDims(boardDims), boardSize(boardSize), mode(mode), path(path), topicColor(topicColor), topicIr(topicIr),
        ...
        lastTime(node->now()), steadyStart(node->now()), lastCentroid(0,0), capturedCentroid(-1000,-1000), isSteady(false)
  {
    if (symmetric)
    {
  ...
  void drawCoverage(cv::Mat &img) { int w=img.cols, h=img.rows; for(int i=1; i<3; ++i) { cv::line(img, cv::Point(i*w/3,0), cv::Point(i*w/3,h), cv::Scalar(0,255,255), 2); cv::line(img, cv::Point(0,i*h/3), cv::Point(w,i*h/3), cv::Scalar(0,255,255), 2); } for(int r=0; r<3; ++r) for(int c=0; c<3; ++c) { if(coverage[r*3+c]>0) { cv::putText(img, "OK", cv::Point(c*w/3+20, r*h/3+50), cv::FONT_HERSHEY_SIMPLEX, 1.5, cv::Scalar(0,255,0), 3); } else { cv::putText(img, "-", cv::Point(c*w/3+20, r*h/3+50), cv::FONT_HERSHEY_SIMPLEX, 1.5, cv::Scalar(0,0,255), 3); } } std::stringstream ss; int covCount=0; for(int k=0; k<9; ++k) if(coverage[k]>0) covCount++; ss << "Coverage: " << (int)((covCount/9.0)*100) << "%"; cv::putText(img, ss.str(), cv::Point(20, h-30), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0,255,0), 2); }   void checkSmartCapture(const std::vector<cv::Point2f> &points, const cv::Mat &color, const cv::Mat &ir, const cv::Mat &irGrey, const cv::Mat &depth, const std::vector<cv::Point2f> &pColor, std::vector<cv::Point2f> &pIr) { if(points.empty()) { isSteady=false; return; } cv::Point2f centroid(0,0); for(auto &p : points) centroid += p; centroid *= (1.0/points.size()); double dist = cv::norm(centroid - lastCentroid); if(dist < 3.0) { if(!isSteady) { steadyStart = node->now(); isSteady=true; } else if((node->now() - steadyStart).seconds() > 2.0) { if((node->now() - lastTime).seconds() > 3.0) { double distFromCap = cv::norm(centroid - capturedCentroid); if(distFromCap > 50.0) { store(color, ir, irGrey, depth, pColor, pIr); lastTime = node->now(); steadyStart = node->now(); capturedCentroid = centroid; OUT_INFO(node, "AUTO-CAPTURE TRIGGERED!"); } } } } else { isSteady = false; } lastCentroid = centroid; }
  void run()
  {
    startRecord();
...
#pragma omp parallel for
  ...
  void callback(const sensor_msgs::msg::Image::ConstSharedPtr imageColor, const sensor_msgs::msg::Image::ConstSharedPtr imageIr, const sensor_msgs::msg::Image::ConstSharedPtr imageDepth) {
    cv::Mat irScaled; if (mode == COLOR || mode == SYNC) readImage(imageColor, color);
    ...
    else
    {
      ...
      if (foundColor)
      {
        cv::cornerSubPix(color, pointsColor, cv::Size(11, 11), cv::Size(-1, -1), termCriteria);
          checkSmartCapture(pointsColor, color, ir, irGrey, depth, pointsColor, pointsIr);
          checkSmartCapture(pointsColor, color, ir, irGrey, depth, pointsColor, pointsIr);
      }
      if (foundIr)
      {
        cv::cornerSubPix(irGrey, pointsIr, cv::Size(11, 11), cv::Size(-1, -1), termCriteria);
          checkSmartCapture(pointsIr, color, ir, irGrey, depth, pointsColor, pointsIr);
          checkSmartCapture(pointsIr, color, ir, irGrey, depth, pointsColor, pointsIr);
      }
    }
  ...
  void display()
  {
    ...
    while (running)
    {
      if (update)
      {
        ...
        switch (mode) { case COLOR: drawCoverage(colorDisp); cv::imshow("color", colorDisp); break; case IR: drawCoverage(irDisp); cv::imshow("ir", irDisp); break; case SYNC: drawCoverage(colorDisp); cv::imshow("color", colorDisp); drawCoverage(irDisp); cv::imshow("ir", irDisp); break; }
  ...
  void store(const cv::Mat &color, const cv::Mat &ir, const cv::Mat &irGrey, const cv::Mat &depth, const std::vector<cv::Point2f> &pointsColor, std::vector<cv::Point2f> &pointsIr)
  {
    ...
    OUT_INFO(node, "Captured frame " << frameNumber << " -> " << base);
    int cx = -1, cy = -1; if(!pointsColor.empty()) { cv::Point2f sum(0,0); for(auto &p : pointsColor) sum += p; cx = (sum.x / pointsColor.size()) / (color.cols / 3.0); cy = (sum.y / pointsColor.size()) / (color.rows / 3.0); } if(cx>=0 && cx<3 && cy>=0 && cy<3) coverage[cy*3+cx]++; OUT_INFO(node, "Coverage Map (3x3):"); for(int r=0; r<3; ++r) { std::stringstream ss; for(int c=0; c<3; ++c) { int count = coverage[r*3+c]; if(count > 0) ss << "[OK] "; else ss << "[  ] "; } OUT_INFO(node, ss.str()); }
...
#pragma omp critical
                           ...
                           cv::Mat &cameraMatrix, cv::Mat &distortion, cv::Mat &rotation, cv::Mat &projection, std::vector<cv::Mat> &rvecs, std::vector<cv::Mat> &tvecs)
  {
    ...
    OUT_INFO(node, "--- Per-Image Analysis ---"); double totalRMS = 0; int outliers = 0; std::vector<int> grid(9, 0); for(size_t i=0; i<points.size(); ++i) { std::vector<cv::Point2f> proj; cv::projectPoints(pointsBoard[i], rvecs[i], tvecs[i], cameraMatrix, distortion, proj); double err = cv::norm(points[i], proj, cv::NORM_L2) / std::sqrt(points[i].size()); totalRMS += err; std::string st = "OK"; if(err > 1.0) { st = "OUTLIER"; outliers++; } OUT_INFO(node, "Img " << i << ": " << err << " px [" << st << "]"); cv::Point2f sum(0,0); for(auto &p : points[i]) sum += p; int gx = (sum.x/points[i].size()) / (size.width/3.0); int gy = (sum.y/points[i].size()) / (size.height/3.0); if(gx>=0 && gx<3 && gy>=0 && gy<3) grid[gy*3+gx]++; } int cov = 0; for(int k=0; k<9; ++k) if(grid[k]>0) cov++; double s_cov = (cov/9.0)*40.0; double s_err = std::max(0.0, 30.0 - (error*20.0)); double s_div = std::min(30.0, (double)points.size()); OUT_INFO(node, "--- QUALITY SCORE: " << (int)(s_cov+s_err+s_div) << "/100 ---");
    std::ofstream csvFile("/workspace/vision_work/calibration_results.csv"); if(csvFile.is_open()) { csvFile << "Frame,RMS_Error,Status,GridX,GridY\n"; for(size_t i=0; i<points.size(); ++i) { std::vector<cv::Point2f> proj; cv::projectPoints(pointsBoard[i], rvecs[i], tvecs[i], cameraMatrix, distortion, proj); double err = cv::norm(points[i], proj, cv::NORM_L2) / std::sqrt(points[i].size()); std::string st = (err > 1.0) ? "OUTLIER" : "OK"; cv::Point2f sum(0,0); for(auto &p : points[i]) sum += p; int gx = (sum.x/points[i].size()) / (size.width/3.0); int gy = (sum.y/points[i].size()) / (size.height/3.0); csvFile << i << "," << err << "," << st << "," << gx << "," << gy << "\n"; } csvFile.close(); OUT_INFO(node, "Data exported to /workspace/vision_work/calibration_results.csv"); } else { OUT_ERROR(node, "Could not open CSV file for writing!"); }
