...
def generate_launch_description():
    ...
    gazebo = IncludeLaunchDescription(
        ...
        # Arguments to run Gazebo headless (no GUI) if needed, or with GUI (default)
        # launch_arguments={'headless': 'false'}.items() 
    )

    # 2. Start the Robot State Publisher to broadcast TF frames
    # This reads the URDF and publishes /tf to map joint angles to link positions
    robot_state_publisher_node = Node(
        ...
        output='screen',
        parameters=[{
            'robot_description': open(os.path.join(parol6_package_path, 'urdf', 'PAROL6_gazebo.urdf')).read(),
        }]
    )
