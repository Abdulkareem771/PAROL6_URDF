...
static float readEncoder(uint8_t idx) {
  ...
  // possible.  This prevents noisy PWM captures from corrupting the
  // multi-turn counter or causing velocity spikes.
  if (joints[idx].last_motor_angle >= 0.0f) {       // skip first reading
    ...
    float max_change = MAX_JOINT_VELOCITIES[idx] * GEAR_RATIOS[idx]
                     * ((float)CONTROL_PERIOD_MS / 1000.0f) * 5.0f;
...
void controlUpdate() {
  for (uint8_t i = 0; i < NUM_MOTORS; i++) {
    ...
    //     Without this, ±0.006 rad noise × Kp=5 = ±0.03 rad/s → motor buzzes
    if (fabsf(j->velocity_command) < 0.02f)
