...
static void parseCommand(const char* cmd) {
  ...
  // ---- SAFETY: Don't accept position commands until ROS has real feedback ----
  // ROS sends hw_command_positions_ = {0,0,...} at startup because hw_state_positions_
  // hasn't been updated with encoder feedback yet. We must ignore ALL commands
  // until ROS has had time to receive our feedback and incorporate it.
  //
  // Timeline: arm → send feedback at 50Hz → ROS receives → updates state →
  // sends real positions. We skip commands for ~1 second to be safe.
  static uint16_t arm_grace_remaining = 0;

  if (!controlIsArmed()) {
    controlArm();
    arm_grace_remaining = 100;  // Skip next 100 commands (~1 sec at 100 Hz ROS rate)
    return;
  }

  if (arm_grace_remaining > 0) {
    arm_grace_remaining--;
    return;  // Still in grace period — ignore stale commands
  }
