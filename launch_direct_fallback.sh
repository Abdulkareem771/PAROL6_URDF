#!/bin/bash
# DIRECT CONTROL SOLUTION
# Bypasses MoveIt Servo completely to guarantee robot control.
...
echo -e "${BLUE}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║      FALLBACK: DIRECT XBOX CONTROL (NO MOVEIT)              ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════════════════════╝${NC}"

# 1. Create the Direct Control Script
echo -e "${YELLOW}[1/2] Creating direct control script...${NC}"
cat > /home/kareem/Desktop/PAROL6_URDF/xbox_direct_control.py << 'EOF'
#!/usr/bin/env python3
import rclpy
...
if __name__ == '__main__':
    main()
EOF
chmod +x /home/kareem/Desktop/PAROL6_URDF/xbox_direct_control.py

# 2. Create Launch Script
echo -e "${YELLOW}[2/2] Creating launch script...${NC}"
cat > /home/kareem/Desktop/PAROL6_URDF/launch_direct_control.sh << 'EOF'
#!/bin/bash
...
wait
EOF
chmod +x /home/kareem/Desktop/PAROL6_URDF/launch_direct_control.sh

echo -e "${GREEN}✓ Direct control scripts created.${NC}"
echo ""
echo -e "${BLUE}Launching Direct Control...${NC}"

# Launch
docker exec -it parol6_dev bash -c "source /opt/ros/humble/setup.bash && source /workspace/install/setup.bash && /workspace/launch_direct_control.sh"
